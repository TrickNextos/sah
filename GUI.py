...
class GUI:
    ...
    def draw_piece(self, pos: Tuple[int, int], piece_path: str, background=Status.EMPTY):
        ...
        img = pygame.transform.scale(
            nonsized_img, (self.SQUARE_SIZE, self.SQUARE_SIZE))
    ...
    def test(self, piece):
        ...
        img = pygame.transform.scale(pygame.image.load(
            piece.img_path), (self.SQUARE_SIZE, self.SQUARE_SIZE))
    ...
    def draw_can_move(self, possible_moves: Dict):
        ...
        for (x, y), status in possible_moves.items():
            if status in (Status.ENEMY, Status.EN_PASSANT):
                self.draw_square(x, y, Status.HIGHLIGHT)
            else:
                pygame.draw.circle(
                    self.screen, self.get_color(x, y).can_move,
                    (x*self.SQUARE_SIZE + self.SQUARE_SIZE * .5,
                     y*self.SQUARE_SIZE + self.SQUARE_SIZE * .5),
                    self.SQUARE_SIZE * .2
                )
